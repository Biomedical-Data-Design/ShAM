...
@author: mikewang
"""
from gan_mask_generate import mask_generate
...
filename = 'a9159b11-ba49-4885-902e-00c8d5095b98.png'
...
def generate_masked_images(I,k, num_parts, save_dir, isSmall):
    '''
    generate all black out images with all different combinations for GAN
    input:
      I: image
      k: player index
      num_parts: total number of players
    ...
    for i in range(len(grey_area_list)):
        ...
        I_m = mask_generate(k, grey_area_list[i], (256, 256, 3))
        # plt.imshow(I_m)
        print(I_m.shape)
        if isSmall == False: 
